...
chroot_cmd apt-key add - < ${SCRIPTSDIR}/../keys/parrot-debian-archive-keyring.gpg 
...
sudo rm "${INSTALLDIR}/etc/apt/sources.list"
sudo cat <<EOF > "${INSTALLDIR}/etc/apt/sources.list.d/parrot.list"  
# ParrotOS repository  
deb http://HTTPS///deb.parrot.sh/parrot/ parrot main contrib non-free
#deb-src http://HTTPS///deb.parrot.sh/parrot/ parrot main contrib non-free
deb http://HTTPS///deb.parrot.sh/parrot/ rolling-security main contrib non-free
#deb-src http://HTTPS///deb.parrot.sh/parrot/ rolling-security main contrib non-free
EOF
