...
function updateDebianSourceList() {
    ...
    source="deb http://HTTPS///deb.debian.org/debian-security ${DEBIANVERSION}${security_suffix} main contrib non-free"
    if ! grep -r -q "$source" "${list}"*; then
        echo -e "$source" >> "${list}"
    fi
...
function updateMintSourceList() {
    ...
    cat > "${INSTALLDIR}/etc/apt/sources.list.d/official-package-repositories.list" <<EOF
     # Do not edit this file manually, use Software Sources instead.
     deb http://packages.linuxmint.com ${MINT_DIST} main upstream import backport
     ...
     deb http://archive.ubuntu.com/ubuntu ${DIST}-security main restricted universe multiverse
...
EOF
    if [[ -n "$USE_QUBES_REPO_VERSION" && $DISTRIBUTION != "qubuntu" ]]; then
        cat >> "${INSTALLDIR}/etc/apt/sources.list.d/qubes-builder.list" <<EOF
deb [arch=amd64] http://HTTPS///deb.qubes-os.org/r${USE_QUBES_REPO_VERSION}/vm $DIST main
EOF
       if [ "0$USE_QUBES_REPO_TESTING" -gt 0 ]; then
          cat >> "${INSTALLDIR}/etc/apt/sources.list.d/qubes-builder.list" <<EOF
deb [arch=amd64] http://HTTPS///deb.qubes-os.org/r${USE_QUBES_REPO_VERSION}/vm ${DIST}-testing main
EOF
        fi
